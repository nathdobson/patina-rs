...
impl<'a> Bimesh2<'a> {
    pub fn new(mesh1: &'a Mesh, mesh2: &'a Mesh) -> Self {
    ...
    pub fn build_new_edges(&mut self) {
        ...
        for (tri1, tri2) in tri_pairs {
            ...
            let mut edge = OrderedMeshEdge::from(new_edge.into_inner().unwrap());
            ...
            for (tri, face) in &mut self.new_faces[mesh].iter_mut().enumerate() {
                ...
                for e in 0..3 {
                    ...
                    for &next in &face.edge_vertices[e] {
                        // face.polygon_edges[color].insert(prev, next);
                ...
                for e in 0..3 {
                    for &nv in &face.edge_vertices[e] {
                        ...
                        loop {
                            ...
                            face.edges.insert(MeshEdge::new(prev, next));
                            // face.polygon_edges[color].insert(prev, next);
                            // face.polygon_edges[(color + 1) % 2].insert(next, prev);
    ...
    pub fn build(&mut self) {
        ...
        println!("{:#?}", self.new_tri_cats);
    }
    pub fn new_mesh(&self, mesh: usize, inside: bool) -> Mesh {
        Mesh::new(
            self.vertices.clone(),
...
async fn test_tetrahedron() -> anyhow::Result<()> {
    ...
    let mut bimesh = Bimesh2::new(&mesh1, &mesh2);
    bimesh.build();
